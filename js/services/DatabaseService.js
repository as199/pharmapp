...
ng.service('DatabaseService', function($q) {
	return {
		...
		getTotalTurnoverByPharma: function(id) {
			var deferred = $q.defer();
			var query = "SELECT prixtotal, prixtotalreduit FROM chiffreaffaire WHERE pharmaid="+id+" GROUP BY prixtotal, prixtotalreduit";
			connection.query(query, function(err, rows) {
				if (err) deferred.reject(err);
				deferred.resolve(rows);
			});
		...
		getTotalSaleByPharma: function(id) {
			var deferred = $q.defer();
			var query = "SELECT prixtotal, prixtotalreduit FROM chiffreaffaire WHERE type='vente' AND pharmaid="+id+" GROUP BY prixtotal";
			connection.query(query, function(err, rows) {
				if (err) deferred.reject(err);
				deferred.resolve(rows);
			});
		...
		getTotalBillingByPharma: function(id) {
			var deferred = $q.defer();
			var query = "SELECT prixtotal, prixtotalreduit FROM chiffreaffaire WHERE type='facturation' AND pharmaid="+id+" GROUP BY prixtotal";
			connection.query(query, function(err, rows) {
				if (err) deferred.reject(err);
				deferred.resolve(rows);
			});
